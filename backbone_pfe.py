...
class Network_D(nn.Module):
    def __init__(self, *args, **kwargs):
        ...
        pretrained_state = torch.load(mod_pth)
...
class Sigmanet(nn.Module):
    def __init__(self, *args, **kwargs):
        super(Sigmanet, self).__init__(*args, **kwargs)

        self.sigmanet_pfe = nn.Sequential(
            nn.Linear(2048, 1024),
            nn.BatchNorm1d(1024),
            nn.ReLU(),
            nn.Linear(1024, 1024),
        ...
        self.sigmanet_pfe.apply(weights_init_kaiming)

    def forward(self, x):
        sigma = self.sigmanet_pfe(x)
        sigma = self.gamma * sigma + self.beta
        # sigma = torch.log(1e-6+torch.exp(sigma))
        sigma = self.softplus(sigma)
        # sigma = torch.exp(sigma)

        if self.training:
            return sigma, self.gamma
        else:
            return sigma
...
class SigmaNet(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Linear(512, 1024, bias=True)
        self.identity = nn.Conv2d(2048, 1024, 1, bias=True)
        self.softplus = nn.Softplus()
