...
noise = '/media/ivy/research/datasets/market1501/random_noise/my_bounding_box_train_0.1_0.h5'
...
class Market1501(Dataset):
    def __init__(self, data_pth, is_train = True, *args, **kwargs):
        ...
        if is_train:
            ...
            # min_l = min(label)
            # max_l = max(label)
            image_name = np.squeeze(image_name)
            img_paths = image_name.tolist()
            cid = cid.tolist()
            label = label.tolist()
            self.is_train = is_train
            self.im_pths = [osp.join(data_pth, str(im,'utf-8')[-23:]) for im in img_paths]
            self.im_infos = {}
            self.person_infos = {}
            for i, im in enumerate(self.im_pths):
                tokens = im[-23:].split('_')
                im_pth = self.im_pths[i]
                pid = label[i]
                pid_raw = int(tokens[0])
                if pid!=pid_raw:
                    cnt += 1
                cam = int(tokens[1][1])
...
if __name__ == "__main__":
    ds_train = Market1501('/media/ivy/research/datasets/market1501/bounding_box_train')
    ds_test = Market1501('/media/ivy/research/datasets/market1501/bounding_box_test', is_train = False)
    im, lb, _ = ds_train[10]
    im, lb, _ = ds_test[10]
    print('done')
